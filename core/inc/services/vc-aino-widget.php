...
class Vc_Aino_Widget {
    ...
    public function __construct() {
        ...
        add_filter('woocommerce_package_rates', array($this, 'remove_shipping_method_from_checkout'), 10, 2);
    }

    public function remove_shipping_method_from_checkout($rates, $package)
    {
            ...
            foreach (WC()->cart->get_shipping_packages() as $package) {            
                ...
                foreach ($zone->get_shipping_methods() as $method) {
                    if (!empty($method->is_vc_aino) && $method->enabled == 'yes') {
                        if(!$this->process_shipping_method($method, $package)) {
                            if($result[0] == $method->vc_aino_id) {
                                unset($rates[$key]);
                            }
                        }
                    }
    ...
    public function select_language() {
        if(function_exists('icl_object_id')) {
            if(ICL_LANGUAGE_CODE === 'da') return 'dk';
